...
namespace DartsMobilApp.ViewModel
{
    ...
    public partial class CounterViewModel : ObservableObject
    {
        ...
        private double MyCheckoutPercentage
        {
            get
            {
                return Math.Round((double)MyPlayerSuccessfulThrownDartsForDouble / MyPlayerAllThrownDartsForDouble, 2) * 100;
            }
        }
        ...
        private async Task SetFirstPlayersPoints(string thrownPoint)
        {
            ...
            if (PointsFirstPlayer == "0")
            {
                ...
                if (ImTheFirst)
                {
                    await ShowCheckOutPopUp(PointsFirstPlayer);
                }
                SetDefaultValues();
            ...
            {
                if (ImTheFirst && int.Parse(PointsFirstPlayer) <= 50 && int.Parse(PointsFirstPlayer) > 0)
                {
                    await ShowCheckOutPopUp(PointsFirstPlayer);
                }
                isFirstPlayer = false;
        ...
        private async Task SetSecondPlayersPoints(string thrownpoint)
        {
            ...
            if (PointsSecondPlayer == "0")
            {
                ...
                if (!ImTheFirst)
                {
                    await ShowCheckOutPopUp(PointsSecondPlayer);
                }
                SetDefaultValues();
            ...
            {
                if (!ImTheFirst && int.Parse(PointsSecondPlayer) <= 50 && int.Parse(PointsSecondPlayer) > 0)
                {
                    await ShowCheckOutPopUp(PointsSecondPlayer);
                }
        ...
        private async Task CheckMatchWinner()
        {
            if (NeedToWinSets != 1)
            {
                ...
                {
                    await HandleMatchWin(true);
                }
                else if (SecondPlayerSetsWon == needToWinSets)
                {
                    await HandleMatchWin(false);
                }
            }
            else
            {
        ...
        private async Task CheckSetWin()
        {
            if (FirstPlayerWonLeg == needToWinLegs)
            {
                FirstPlayerSetsWon++;
        ...
        private async Task HandleMatchWin(bool isFirstPlayerWinner)
        {
            string winnerName = isFirstPlayerWinner ? StartingPlayerName : SecondPlayerName;
            int winnerSets = isFirstPlayerWinner ? FirstPlayerSetsWon : SecondPlayerSetsWon;
            int loserSets = isFirstPlayerWinner ? SecondPlayerSetsWon : FirstPlayerSetsWon;
            int winnerLegs = isFirstPlayerWinner ? AllWonLegsOnTheMatchFirst : AllWonLegsOnTheMatchSecond;
            int loserLegs = isFirstPlayerWinner ? AllWonLegsOnTheMatchSecond : AllWonLegsOnTheMatchFirst;
            if (winnerSets == 0)
            {
                await TextSpeach($"{winnerName} nyerte a mérkőzést {winnerLegs}-{loserLegs} arányban!");
            }
            else
            {
                await TextSpeach($"{winnerName} nyerte a mérkőzést {winnerSets}-{loserSets} arányban!");
            }
            var model = CreateEndMatchModel(isFirstPlayerWinner);
            if (IsFriendlyMatch)
                await _signalRService.EndFriendlyMatch(MatchId, SecStoreItems.UserId, model);
            else
        ...
        {
            return new EndMatchModel
            {
                Legs = NeedToWinSets == 1 ? allPlayedLegs : AllPlayedLegsOnTheMatch,
                LegsWon = isFirstPlayer ? AllWonLegsOnTheMatchFirst : AllWonLegsOnTheMatchSecond,
                Sets = allPlayedSet,
                SetsWon = isFirstPlayer ? FirstPlayerSetsWon : SecondPlayerSetsWon,
                Averages = MyAverage,
                CheckoutPercentage = MyCheckoutPercentage,
                HighestCheckout = MyHighestCheckOut,
                Max180s = MyPlayer180s,
                NineDarter = MyNineDarters,
                Won = isFirstPlayer == ImTheFirst
            };
        }


        private async Task ShowCheckOutPopUp(string points)
        {
            try
            {
                await MainThread.InvokeOnMainThreadAsync(async () =>
                {
                    if (int.Parse(points) <= 50 && int.Parse(points) > 0)
                    {
                        var popup = new SimpleCheckoutPopUp();
                        await Application.Current.MainPage.ShowPopupAsync(popup);
                    }
                    else if (int.Parse(points) == 0)
                    {
